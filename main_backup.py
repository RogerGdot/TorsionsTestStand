...
try:
    ...
    print("PyModbus nicht verfügbar - N5 Nanotec Steuerung nicht möglich")
...
class N5NanotecController:
    ...
    def connect(self) -> bool:
        ...
        if self.demo_mode:
            print(f"[DEMO] N5 Nanotec Controller - Simulation aktiv")
    ...
    def is_position_reached(self, target: float, tolerance: float = 0.5) -> bool:
        """
        Prüft, ob eine Zielposition erreicht wurde.
        
        Args:
            target (float): Zielposition in Grad
            tolerance (float): Toleranz in Grad
        
        Returns:
            bool: True wenn Position erreicht
        """
...
class MainWindow(QMainWindow):
    ...
    def get_current_torque_scale(self) -> float:
        """Gibt den Torque-Scale-Wert aus den Konstanten zurück."""
        return TORQUE_SCALE

    def get_current_torque_channel(self) -> str:
        """Gibt den Torque-Kanal aus den Konstanten zurück."""
        return DAQ_CHANNEL_TORQUE
